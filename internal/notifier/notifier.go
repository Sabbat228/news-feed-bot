...
import (
	...
	"io"
	"net/http"
	markup "news-feed-bot/internal/botkit/markup"
...
func New(
	articleProvider ArticleProvider, // Исправлено: интерфейс вместо конкретной реализации
	summarizer Summarizer,
	bot *tgbotapi.BotAPI,
	sendInterval time.Duration,
	lookupTimeWindow time.Duration,
...
func (n *Notifier) sendArticle(article model.Article, summary string) error {
	...
	escapedTitle := markup.EscapeForMarkdown(article.Title)
	escapedSummary := markup.EscapeForMarkdown(summary)
	escapedLink := markup.EscapeForMarkdown(article.Link)
